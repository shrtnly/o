...
async function syncCourse() {
  ...
  try {
    ...
    // Chapter upsert in chunks of 50 to avoid payload size limit
    ...
    // Learning Points upsert in chunks of 50
    ...
    const { error: qsErr } = await supabase.from('mcq_questions').upsert(questionsToUpsert);
    ...
    const { error: optsErr } = await supabase.from('mcq_options').upsert(optionsToUpsert);
    ...
    if (currentQuestionIds.length > 0) {
      const dbOptions = [];
      const questionChunks = chunkArray(currentQuestionIds, CHUNK_SIZE);
      ...
      if (optionsToDelete.length > 0) {
        ...
        const optionChunks = chunkArray(optionsToDelete, CHUNK_SIZE);
    ...
    if (currentLpIds.length > 0) {
      ...
      if (questionsToDelete.length > 0) {
        ...
        const questionChunks = chunkArray(questionsToDelete, CHUNK_SIZE);
        for (const chunk of questionChunks) {
          const { error: delErr } = await supabase
            .from('mcq_questions')
            .delete()
    ...
    if (currentChapterIds.length > 0) {
      ...
      if (lpsToDelete.length > 0) {
        ...
        const lpChunksToDelete = chunkArray(lpsToDelete, CHUNK_SIZE);
        for (const chunk of lpChunksToDelete) {
          const { error: delErr } = await supabase
            .from('learning_points')
            .delete()
            .in('id', chunk);
    ...
    if (currentUnitIds.length > 0) {
      ...
      if (chaptersToDelete.length > 0) {
        ...
        const chapterChunksToDelete = chunkArray(chaptersToDelete, CHUNK_SIZE);
        for (const chunk of chapterChunksToDelete) {
          const { error: delErr } = await supabase
            .from('chapters')
            .delete()
            .in('id', chunk);
    ...
    if (unitsToDelete.length > 0) {
      ...
      const unitChunksToDelete = chunkArray(unitsToDelete, CHUNK_SIZE);
      for (const chunk of unitChunksToDelete) {
        const { error: delErr } = await supabase
          .from('units')
          .delete()
          .in('id', chunk);
    ...
    console.log('★ Sync completed successfully! Live database matches JSON.');
