...
const COURSE_ID = '9372bb76-c1eb-4c2f-bcde-96400a579880';
...
async function syncPasswordCourse() {
  const sourcePath = path.join(process.cwd(), 'database', 'courses', 'password_and_account_security.json');
  ...
  try {
    ...
    fs.writeFileSync(path.join(process.cwd(), 'scratch', 'password_and_account_security_backup.json'), finalJsonString, 'utf-8');
    
    console.log('✓ Local files updated successfully:');
    console.log('  - database/courses/password_and_account_security.json');
    console.log('  - scratch/password_and_account_security_backup.json');
    console.log('★ Password & Account Security Course sync completed successfully!');
