...
const ChapterSection = ({ moduleId }) => {
    ...
    return (
        <div className="space-y-3">
            {chapters.map((chapter) => (
                <div key={chapter.id} className="border border-slate-200 rounded-lg overflow-hidden bg-white">
                    <div className="flex items-center gap-4 p-3 hover:bg-slate-50 transition-colors">
                        ...
                        <input
                            type="text"
                            className="flex-1 bg-transparent border-none p-0 font-bold text-slate-800 text-sm outline-none focus:ring-0"
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <input
                                type="text"
                                placeholder="Time"
                                className="w-16 bg-transparent border-none p-0 text-xs font-medium text-slate-500 outline-none"
                                defaultValue={chapter.estimated_time || ''}
                                onBlur={(e) => handleUpdateChapter(chapter.id, { estimated_time: e.target.value })}
                            />
                            <button onClick={() => handleDeleteChapter(chapter.id)} className="p-1 text-slate-300 hover:text-red-500"><Trash2 size={14} /></button>
                            <button
                                onClick={() => setExpandedChapterId(expandedChapterId === chapter.id ? null : chapter.id)}
                                className={cn("p-1.5 rounded text-slate-400", expandedChapterId === chapter.id ? "bg-slate-200 text-slate-900" : "hover:bg-slate-100")}
                            >
            ...
            <button onClick={handleAddChapter} className="flex items-center gap-2 text-slate-400 hover:text-slate-900 font-bold text-xs p-2">
