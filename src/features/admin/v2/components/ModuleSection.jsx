...
const ModuleSection = ({ courseId }) => {
    ...
    return (
        <div className="space-y-4">
            {modules.map((module) => (
                <div key={module.id} className="border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden bg-white dark:bg-slate-900/50 shadow-sm">
                    <div className="flex items-center gap-4 p-4 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                        <GripVertical size={16} className="text-slate-300 dark:text-slate-700 cursor-grab" />
                        <input
                            type="text"
